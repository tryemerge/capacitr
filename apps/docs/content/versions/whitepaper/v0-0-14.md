---
title: "EmMittr Whitepaper v0.0.14"
version: "0.0.14"
date: "2026-02-18"
document: whitepaper
---

# EmMittr Whitepaper v0.0.14

::: info Archive
This is an archived snapshot of the EmMittr Whitepaper at version 0.0.14. For the current version, see the [latest whitepaper](/whitepaper/).
:::
...
v0.0.14 --- February 2026

DRAFT --- For Discussion Only

*EmMittr is simple, and it is extremely powerful. We have a lot to discuss. But first, allow us to unbury the lead and show you what is possible with this project.*
...
**EmMittr** introduces a new primitive for app tokens: **Work → Emmitt → Earn.** Projects define what counts as valuable work. Users who do that work earn **"emmissions"** --- derivative tokens representing a staked position in a fee-earning LP pool. The result is a token where participation is directly tied to economic reward.
...
**Incentive structure:** Not a decay curve. **Winner-take-all.** Participants submit work. The project evaluates it. The best submissions are rewarded. The rest receive nothing. This is not "participate and earn" --- it is "compete and get hired." Emmissions for qualitative work are distributed as bounties, rounds, and discrete payouts from a defined pool.
...
The delta between free submission and valuable emmissions is not a flaw. It is **the project investing its own reputation for distribution.** Emmissions come from the reserve. The reserve is backed by LP. If the project distributes poorly --- rewards garbage, gets sybiled --- the market sees bad metrics, the token price drops, and the project's emmissions become worthless. Reputation is the accountability mechanism.

The winner-take-all structure changes the attack math. Spinning up 300 wallets doesn't help if the project selects on quality. Self-dealing destroys the metrics that attract speculators. Collusion is self-limiting because it degrades the token's value. The system doesn't prevent every attack. It makes attacks expensive and self-defeating.
...
EmMittr creates a new primitive: tokens where participation is directly rewarded through emmissions --- derivative tokens representing staked positions in fee-earning LP pools.

The default Work metric means every token has participation economics from day one. The SDK lets projects wire custom Work for deeper incentives. And agents complete the loop --- launching, serving, measuring, rewarding, and growing their own economies autonomously.

For creators, it's sustainable revenue without dilution. For participants, it's earning from the ecosystems they promote and use. For agents, it's the economic layer that makes autonomous growth possible. For everyone, it's a fairer way to distribute the value that participation creates.
...
The act of triggering an emmission when Work is performed. Reserve tokens move to pool, user receives em{Token}. The core loop: Work → Emmitt → Earn.

## EmMittr

The protocol for participation-based token economics. Three tiers: default Work metric, custom Work via SDK, and agent-native autonomous economics.
...
Any action that creates value for a project and triggers emmission distribution. Default: quality-scored participation metric (available on every launch). Custom: any event reported via SDK. Agent: skill invocation, automatically measured and reported.
