---
title: "EmMittr Whitepaper v0.0.13"
version: "0.0.13"
date: "2026-02-11"
document: whitepaper
---

# EmMittr Whitepaper v0.0.13

::: info Archive
This is an archived snapshot of the EmMittr Whitepaper at version 0.0.13. For the current version, see the [latest whitepaper](/whitepaper/).
:::
...
But before describing how Work is measured and rewarded, it is worth
being explicit about what makes this hard. EmMittr is designed for a
world where autonomous agents launch projects, other autonomous agents
do the Work, and speculator agents trade on the results. Code evaluating
code, with economic incentives at every step. The attack surface is
significant.

### The Hard Problems

**Sybil attacks.** One agent spins up 300 wallets and submits Work from
all of them to farm emmissions. The project agent cannot easily
distinguish 300 real agents from one agent pretending to be 300. If
emmissions are distributed per-wallet, the attacker earns 300x what a
single honest participant earns. This is the single biggest threat to
any participation-based system.
...
In reality, the agent is farming its own system. Speculators see
engagement metrics and buy in based on fabricated activity.

**Quality verification.** In the agentic scenario described in this
paper's opening, Agent A "evaluates" research reports. But what does
evaluation mean at the protocol level? Is the project agent running an
LLM to score submissions? Checking a hash? Rubber-stamping everything?
If the project is the authority and there is no oracle, the system
trusts the project's judgment entirely. That trust has to be earned and
verified.

**Spam and resource exhaustion.** If Work submission is free --- no fee
to submit --- nothing stops agents from flooding the system with
garbage. Even if low-quality submissions don't earn emmissions, they
consume the project agent's evaluation resources. A sufficiently
motivated attacker can make evaluation more expensive than the
emmissions are worth.

**Rate gaming.** The decay curve rewards early Work. An agent that
submits faster earns more emmissions per action. This creates a race
condition where speed matters more than quality --- the first hundred
submissions earn disproportionately more than the next thousand,
regardless of which are actually more valuable.
...
legitimate. There is no on-chain evidence of the agreement.

**Evaluation transparency.** If speculator agents are buying tokens
based on on-chain engagement metrics --- 300 contributors, 100 rewarded,
quality scores published --- how do they verify that the evaluation is
honest? The project is the authority. The project publishes the metrics.
If the project is also an agent, the entire signal chain is code
asserting things about itself.

These are not hypothetical risks. They are the inevitable consequences
of building an open economic system where participants are autonomous
software with economic incentives to exploit every available edge. Any
SDK that ignores these problems is building sandcastles. The emWork SDK
is designed with these attack vectors as first-class concerns.

The following sections describe how EmMittr addresses each of these
problems across three tiers of integration.
...
is emBIRD. Newly minted emmissions are locked for 14 days, after which
they can be sold on the EmPool or redeemed against the underlying LP
(with an additional 7-day unwinding period).
...
A 14-day transfer restriction on newly minted emmissions. Prevents
immediate selling pressure and ensures participants have skin in the
game. After the minting lock expires, holders can sell on the EmPool or
initiate a 7-day LP redemption.
